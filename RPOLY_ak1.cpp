...
void RealIT_ak1(bool* iFlag, int* NZ, double* sss, int N, double p[MDP1], int NN, double qp[MDP1], double* szr, double* szi, double K[MDP1], double qk[MDP1]);
...
int main()
{
	...
	cout << "                                           rpoly_ak1 (23 February 2020)\n";
	cout << "=========================================================================== \n";
	...
	if (toupper(rflag) == 'Y') {
		...
		// Initialize the arrays to Quiet NaN
		for (i = 0; i < MAXDEGREE; ++i)  zeroi[i] = zeror[i] = op[i] = std::numeric_limits<double>::quiet_NaN();
		op[MAXDEGREE] = std::numeric_limits<double>::quiet_NaN();

		//Input the polynomial coefficients from the file and put them in the op vector

		for (i = 0; i < (Degree + 1); ++i) in >> op[i];
